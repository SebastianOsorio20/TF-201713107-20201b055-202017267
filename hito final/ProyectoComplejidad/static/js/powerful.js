(async function () {
  ...
  const scalecolor = d3.scaleLinear()
    .domain(extentw)
    .range([100, 0]);
  const color = d => `hsla(${scalecolor(d[2])}, 100%, 50%, 0.5)` //100%, 50%, 0.5
  ...
  dealWithPath(paths.bestpath, "rgba(0, 128, 0, 0.5)") //"rgba(0, 128, 0, 0.5)" 
  dealWithPath(paths.path1, "rgba(255, 165, 0, 0.5)")//rgba(255, 165, 0, 0.5)
  dealWithPath(paths.path2, "rgba(220,  20, 60, 0.5)")//"rgba(220,  20, 60, 0.5)"
